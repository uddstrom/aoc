...
function connect(p, q) {
    // p and q are in the same cicuite
    ...
    if (cp >= 0 && cq >= 0) {
        // console.log("Both connected but in different curcuites");
...
makeConnections(1000);
C.sort((a, b) => b.size - a.size);
// console.log(C);

var p1 = C.slice(0, 3).reduce((acc, curr) => acc * curr.size, 1);
